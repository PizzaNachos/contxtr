<script lang="ts">
    import { fade } from "svelte/transition";
    import type {SentenceType, WordType} from "../../routes/aapi/types"

    export let me : SentenceType | any;
    export let target: WordType | any;
    export let update_function : Function;
    export let loading_w : Boolean = false;

    let input_val = ""
    // if (!me || !target){
    //     me={
    //         text:["Error", "Broken"],
    //         translation:["Error","Im","Broke"],
    //     }
    //     target={
    //         word:"I'm",
    //         reverse:false
    //     }
    // }
    function check(e:any){
        let i = e?.target?.value
        if (target.reverse){
            if (i == target.word){
                shown = 2
            }
        } else {
            if (i == me.match_regex){
                shown = 2
            }
        }
 
    }
    let shown = 0
    function show_translation(){
        if (shown < 2){
            shown += 1;
        }
    }
    function update(level){
        input_val = '';
        shown = 0;
        update_function(level)
    }
</script>
{#if target.reverse == true}
<div class=card>
	<span class:loading_w> 
        {me.translation[0]} <span class="target"> {me.translation[1]} </span> {me.translation[2]} 
    </span>
    <input 
        class={shown == 2 ? "disabled" : ""} 
        type="text" 
        on:input={(e) => check(e)} disabled={(shown == 2)} 
        bind:value={input_val}
    /> 
    <div>
        {#if shown == 0}
            <button on:click={() => show_translation()}>
                Show
            </button>
        {:else if shown == 1}
            <button on:click={() => show_translation()}>
                Show All
            </button>  
        {/if}
    </div>

    {#if shown > 0}
        <span class="translation" in:fade>        
            {me.text[0]} 

            {#if shown == 1}
                {"_".repeat(target.word.length)} 
            {:else if shown == 2}
                {target.word} 
            {/if}

            {me.text[1]}
        </span>
    {/if}

    <div class="finish_buttons" in:fade>
        {#if shown == 2}
            <button class="next_button" on:click={() => {shown = 0; update(1)}}>
                Fail
            </button>
            <button class="next_button" on:click={() => {shown = 0; update(2)}}>
                Hard
            </button>
            <button class="next_button" on:click={() => {shown = 0; update(3)}}>
                Good
            </button>
            <button class="next_button" on:click={() => {shown = 0; update(4)}}>
                Easy
            </button>
        {/if}
    </div>
</div>
{:else}
<div class=card>
	<span class:loading_w> 
        {me.text[0]} <span class="target"> {target.word} </span> {me.text[1]} 
    </span>
    <input class={shown == 2 ? "disabled" : ""} type="text" on:input={(e) => check(e)} disabled={shown == 2} bind:value={input_val}/> 
    <div>
            {#if shown == 0}
                <button on:click={() => show_translation()}>
                    Show
                </button>
            {:else if shown == 1}
                <button on:click={() => show_translation()}>
                    Show All
                </button>  
            {/if}
    </div>

    {#if shown > 0}
        <span class="translation" in:fade>        
            {me.translation[0]} 

            {#if shown == 1}
                {"_".repeat(target.word.length)} 
            {:else if shown == 2}
                {me.translation[1]} 
            {/if}

            {me.translation[2]}
        </span>
    {/if}

    <div class="finish_buttons" in:fade>
        {#if shown == 2}
            <button class="next_button" on:click={() => {shown = 0; update(1)}}>
                Fail
            </button>
            <button class="next_button" on:click={() => {shown = 0; update(2)}}>
                Hard
            </button>
            <button class="next_button" on:click={() => {shown = 0; update(3)}}>
                Good
            </button>
            <button class="next_button" on:click={() => {shown = 0; update(4)}}>
                Easy
            </button>
        {/if}
    </div>
</div>
{/if}


<style>
    span.target{
        color: rgb(200,200,250);
        font-weight: 500;
    }
	.card{
        font-size: 1.5rem;
		border-radius: 10px;
		background-color: rgb(40,40,40);
        padding: 1em 2em;
        display: flex;
        flex-direction: column;
        justify-content: flex-start;
        align-items: flex-start;
        gap: 1em;
        box-shadow: 0 0 10px black;
	}
    .card span{
        transition: all .25s ease-in-out;
    }
    input{
        padding: 1ch;
        font-size: 1em;
        border-radius: 10px;
        background-color: rgb(20,20,20);
        color: white;
        border: 2px solid black;
        outline: none;
        transition: all .25s ease;
        width: 100%;
    }
    .finish_buttons{
        height: 3em;
        display: flex;
        flex-direction: row;
        justify-content: flex-start;
        align-items: flex-start;
        gap: 1em;
    }
    .translation{
        overflow-wrap: break-word;
    }
    button{
        font-size: .85em;
        padding: .5em;
        border-radius: 5px;
        background-color: rgb(60,60,60);
        color: white;
        transition: all .5s ease;
        border: 2px solid rgba(0,0,0,0);
    }
    button:hover{
        border: 2px solid black;
    }
    .next_button{
        background-color: rgb(80,80,80);
    }
    input.disabled{
 		background-color: rgb(35,35,35);
        border: 2px solid rgba(0,0,0,0);
    }
    .loading_w{
        opacity: 0;
    }
</style>